...
module.exports = function (LocalOptions) {
    ...
    let mergeNodesTimer = (index, newValue, Name) => {
        if (newValue._Timers) {
            ...
            for (let t of newValue._Timers) {
                ...
                let xt = Nodes[index]._Timers[t_index];
                ...
                if (xt.path.startsWith("$."))
                    node_timers[index][t.path] = setTimeout(function () {
    ...
    let mergeNodesSwitch = (index, newValue, Name) => {
    ...
    let mergeNodesMdnsManual = (index, newValue, Name) => {
        if (newValue.Schema == 1) {
    ...
    let findCandidates = (val) => {
        let r;
        r = Nodes.findIndex(n => n.Name == val.Name);
        if (r == -1)
            r = Nodes.findIndex(n => n.Mac == val.Mac);
        if (r == -1)
            r = Nodes.findIndex(n => (n.Macs && n.Macs.includes(val.Mac)) || (val.Macs && val.Macs.includes(n.Mac)));
        if (r == -1)
            r = Nodes.findIndex(n => n.IP == val.IP);
        if (r == -1)
            r = Nodes.findIndex(n => (n.OtherIPs && n.OtherIPs.includes(val.IP)) || (val.OtherIPs && val.OtherIPs.includes(n.IP)));
        return r;
    };
            ...
            if (list && list.dataRef) {
                ...
                if (friend.length == 1 && friend[0]) {
                    ...
                    if (listPort >= 0
                        ...
                        if (!listNode.Errors.Ports[friendPort])
                            listNode.Errors.Ports[friendPort] = {};
                        ...
                        if (!friendNode.Errors.Ports[listPort])
                            friendNode.Errors.Ports[listPort] = {};
    ...
    let loadStaticConfig = () => {
        try {
            let file = fs_1.readFileSync("devices.json");
            let Data = JSON.parse(file.toString());
            for (let p of Data) {
                if (p.Name) {
                    if (p.Macs)
                        for (let i = 0; i < p.Macs.length; i++)
