...
function convertMikrotikString(input) {
...
async function getSystemInfo() {
    try {
        ...
        let parsedData = eval('(' + body + ')');
        if (parsedData.id)
            Switch.Name = convertMikrotikString(parsedData.id);
        if (parsedData.temp)
            Switch.System.CPUTemps = [parsedData.temp];
        if (parsedData.mac)
            Switch.Mac = parsedData.mac;
        if (parsedData.igmp) {
            Switch.Multicast = 'on';
        }
        else {
